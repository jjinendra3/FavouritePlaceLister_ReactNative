...
import {useCameraPermissions,PermissionStatus,launchImageLibraryAsync} from 'expo-image-picker'
...
const Imager = () => {
  ...
  const veryifycam=async()=>{
    ...
    if(cameraPermissionInformation===PermissionStatus.DENIED){
      Alert.alert("Camera Permissions",'Please grant Camera Permissions to run the app.')
      return false;
    }
    return true;
  }
  ...
  return (
    <View>
      <View style={styles.preview}>
       {first!=='' ? <Image source={{uri:first}} 
        style={{ height: "100%", width: "100%", borderRadius: 4 }}/>:<Text>No Image to Show!</Text>}
      </View>
